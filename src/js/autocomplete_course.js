    ...
    if (checked) {
      for (var key = 0; key < option.length; key++) {
        if (option[key.toString()].value) {
          filterSlotArr.indexOf(option[key.toString()].value) === -1 &&
            filterSlotArr.push(option[key.toString()].value);
        } else {
          ...
          for (
            ...
            if (allSelectOption[innerkey.toString()].value) {
              filterSlotArr.indexOf(
                ...
                filterSlotArr.push(allSelectOption[innerkey.toString()].value);
            }
          }
        }
      }
